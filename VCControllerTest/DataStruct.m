...
{
    return [DataStruct createDataStructWithObject:object withStructVersion:nil andDataVersion:nil];
}
                            ...
                            andDataVersion:(NSString *)dataVersion;
{
    ...
    DataStruct *dataStruct = [[DataStruct alloc] init];
    ...
    [DataStruct structIno:dictionaryStuctInfo dataValue:dictionaryDataValue forClassType:[object class] andKey:kDataStructRootKey andVaule:object withDataStruct:dataStruct];
    ...
    if ([AppInfo appVID] != nil)
    {
        [dictionaryBinary setObject:[AppInfo appVID] forKey:kDataStructAppVIDKey];
    }
    ...
    NSData *data = [NSPropertyListSerialization dataWithPropertyList:dictionaryBinary format:NSPropertyListBinaryFormat_v1_0 options:0 error:NULL];
    ...
    [dataStruct setAppVID:[AppInfo appVID]];
...
+ (instancetype)createDataStructWithData:(NSData *)data
{
    ...
    NSDictionary *dictionaryData = [NSPropertyListSerialization propertyListWithData:data options:0 format:&propertyListFormat error:nil];
...
+ (void)structIno:(NSMutableDictionary *)dictionaryStructInfo dataValue:(NSMutableDictionary *)dictionaryDataValue forClassType:(Class)classType andKey:(NSString *)key andVaule:(id)object withDataStruct:(DataStruct *)dataStuct
{
    // 基本类型－－元数据
    if ([DataStruct isBaseDataType:classType] == YES)
    {
        [DataStruct baseDataStructIno:dictionaryStructInfo baseDataDataValue:dictionaryDataValue forClassType:classType andKey:key andVaule:object];
    }
    // 容器类型－－数组、字典
    if ([DataStruct isContainerType:classType] == YES)
    {
        ...
        [DataStruct containerStructIno:dictionaryStructInfo containerDataValue:dictionaryDataValue forClassType:classType andKey:key andVaule:object withDataStruct:dataStuct];
        [[dataStuct arrayCycleCheck] removeLastObject];
    }
    // 自定义类型
    if ([DataStruct isCustomType:classType] == YES)
    {
        if ([[dataStuct arrayCycleCheck] containsObject:object] == YES)
        {
            return;
        ...
        [[dataStuct arrayCycleCheck] addObject:object];
        [DataStruct customStructIno:dictionaryStructInfo customDataValue:dictionaryDataValue forClassType:classType andKey:key andVaule:object withDataStruct:dataStuct];
        [[dataStuct arrayCycleCheck] removeLastObject];
    }
}

// 处理基本类型
+ (void)baseDataStructIno:(NSMutableDictionary *)dictionaryStructInfo baseDataDataValue:(NSMutableDictionary *)dictionaryDataValue forClassType:(Class)classType andKey:(NSString *)key andVaule:(id)object
{
    ...
    if ([DataStruct isBaseDataType:classType] == YES)
    {
        ...
        Class standardType = [DataStruct standardBaseDataType:classType];
        ...
        id standerdObject = [DataStruct standardBaseDataValueWithObject:object];
...
// 处理容器类型
+ (void)containerStructIno:(NSMutableDictionary *)dictionaryStructInfo containerDataValue:(NSMutableDictionary *)dictionaryDataValue forClassType:(Class)classType andKey:(NSString *)key andVaule:(id)object withDataStruct:(DataStruct *)dataStuct
{
    if (classType == nil || object == nil)
    {
        return;
        ...
        if (arrayObject != nil && [arrayObject count] > 0)
        {
            for (NSInteger i = 0; i < [arrayObject count]; ++i)
            {
                ...
                if (obj != nil)
                {
                    ...
                    [DataStruct structIno:structInfo dataValue:dataVaule forClassType:[obj class] andKey:objectKey andVaule:obj withDataStruct:dataStuct];
                }
            }
        }
        [structInfo setObject:NSStringFromClass(classType) forKey:kDataStructRealTypeKey];
        ...
        if (dictionaryObject != nil)
        {
            for (id dictionaryKey in dictionaryObject)
            {
                if (dictionaryKey != nil && [dictionaryKey isKindOfClass:[NSString class]] == YES)
                {
                    ...
                    [DataStruct structIno:structInfo dataValue:dataVaule forClassType:[dictionaryValue class] andKey:(NSString *)dictionaryKey andVaule:dictionaryValue withDataStruct:dataStuct];
                }
            }
        }
        [structInfo setObject:NSStringFromClass(classType) forKey:kDataStructRealTypeKey];
...
// 处理自定义类型
+ (void)customStructIno:(NSMutableDictionary *)dictionaryStructInfo customDataValue:(NSMutableDictionary *)dictionaryDataValue forClassType:(Class)classType andKey:(NSString *)key andVaule:(id)object withDataStruct:(DataStruct *)dataStuct
{
    if (classType == nil || object == nil)
    {
        return;
        ...
        for (unsigned int i = 0; i < propertyCount; ++i)
        {
            ...
            if ([propertyAttributes hasPrefix:@"T@"] == YES)
            {
                ...
                if (iVar != nil)
                {
                    ...
                    {
                        [DataStruct structIno:structInfo dataValue:dataValue forClassType:[propertyIVar class] andKey:propertyName andVaule:propertyIVar withDataStruct:dataStuct];
...
{
    if ([DataStruct standardBaseDataType:classType] != nil)
    {
        return YES;
    }
...
+ (id)standardBaseDataValueWithObject:(id)object
{
    ...
    if (object == nil || [DataStruct isBaseDataType:classType] == NO)
    {
        return nil;
    }
...
{
    if ([DataStruct standardContainerType:classType])
    {
        return YES;
    }
...
{
    if ([DataStruct standardCustomType:classType] != nil)
    {
        return YES;
    }
...
- (id)objectWithStructInfo:(NSDictionary *)dictionaryStructInfo DataValue:(id)dataValue
{
    ...
    if ([DataStruct isBaseDataType:realClassType] == YES)
    {
        return [self baseObjectWithStructInfo:dictionaryStructInfo DataValue:dataValue];
    }
    
    
    if ([DataStruct isContainerType:realClassType] == YES)
    {
        return [self containerObjectWithStructInfo:dictionaryStructInfo DataValue:dataValue];
    }
    
    if ([DataStruct isCustomType:realClassType] == YES)
    {
        return [self customObjectWithStructInfo:dictionaryStructInfo DataValue:dataValue];
    }
...
- (id)baseObjectWithStructInfo:(NSDictionary *)dictionaryStructInfo DataValue:(id)dataValue
{
...
- (id)compatiblePropertyForClassType:(Class)classType WithStrorageObject:(id)strorageObject
{
    ...
    if ([DataStruct isBaseDataType:classType] == YES)
    {
        if ([DataStruct isBaseDataType:[strorageObject class]] == YES)
        {
    ...
    if ([DataStruct isContainerType:classType] == YES)
    {
        if ([DataStruct isContainerType:[strorageObject class]] == YES)
        {
            if ([strorageObject isKindOfClass:classType])
            {
                return strorageObject;
